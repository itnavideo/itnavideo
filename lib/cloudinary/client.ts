...
export function getAssetsByFolder(folder: string): CloudinaryAsset[] {
  if (folder === 'images' || folder === 'Assets/images' || folder === '16:9 Images Library' || folder === 'Reels Images') {
    return catalog['Assets/images'] || catalog['images'] || catalog['16:9 Images Library'] || catalog['Reels Images'] || [];
  }
  return catalog[folder] || catalog[`Assets/${folder}`] || [];
}
...
export function getSfxUrl(keyword: string): string | null {
  const sfxList = catalog['Assets/SFX'] || catalog['SFX'] || [];
...
export function getBackgroundMusicUrl(mood?: string): string | null {
  const musicList = catalog['Assets/Background Music'] || catalog['Background Music'] || [];
...
export function getBackgroundImageUrl(style?: string): string | null {
  const bgList = catalog['Assets/background images'] || catalog['background images'] || [];
...
export function getWebsiteImageUrl(name: string): string | null {
  const siteImages = catalog['Website/images'] || catalog['Website use images'] || [];
