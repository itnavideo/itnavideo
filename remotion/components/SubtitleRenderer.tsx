...
export const SubtitleRenderer: React.FC<SubtitleRendererProps> = ({
  ...
  const renderStyle = () => {
    switch (resolvedConfig.style) {
      ...
      case 'm3-tonal-pill':
        return <M3TonalPillStyle caption={captionWithWords} config={resolvedConfig} fontSize={fontSize} activeWord={activeWord} />;
      case 'm3-dynamic-chip':
        return <M3DynamicChipStyle caption={captionWithWords} config={resolvedConfig} fontSize={fontSize} activeWord={activeWord} />;
      case 'm3-elevated-card':
        return <M3ElevatedCardStyle caption={captionWithWords} config={resolvedConfig} fontSize={fontSize} activeWord={activeWord} />;
      default:
        return <NormalStyle caption={captionWithWords} config={resolvedConfig} fontSize={fontSize} />;
    }
  };
...
// Style: M3 TONAL PILL — Google Material 3 Elevated Full Pill with Primary Container active word
const M3TonalPillStyle: React.FC<StyleProps> = ({caption, config, fontSize, activeWord}) => {
  const words = caption.words ?? [];
  const pillSize = Math.max(44, Math.min(fontSize * 0.9, 74));
  const accentColor = config.highlightColor || '#38BDF8';
  return (
    <div
      style={{
        display: 'inline-flex',
        ...
        boxShadow: '0 12px 36px rgba(0, 0, 0, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.22)',
      }}
    >
          ...
          return (
            <span
              ...
              style={{
                ...
                fontFamily: config.fontFamily,
                color: isActive ? accentColor : '#F8FAFC',
                letterSpacing: '-0.02em',
                padding: isActive ? '2px 14px' : '2px 4px',
                borderRadius: 9999,
                background: isActive ? `${accentColor}24` : 'transparent',
                border: isActive ? `1px solid ${accentColor}66` : '1px solid transparent',
                boxShadow: isActive ? `0 0 20px ${accentColor}33, inset 0 1px 0 rgba(255, 255, 255, 0.25)` : 'none',
                transform: isActive ? 'scale(1.06)' : 'scale(1)',
                transition: 'all 0.1s cubic-bezier(0.05, 0.7, 0.1, 1.0)',
              }}
            >
              {w.word}
            </span>
...
// Style: M3 DYNAMIC CHIP — Google Material 3 Assist & Filter Chip container tokens
const M3DynamicChipStyle: React.FC<StyleProps> = ({caption, config, fontSize, activeWord}) => {
  const words = caption.words ?? [];
  const chipSize = Math.max(40, Math.min(fontSize * 0.84, 68));
  const accentColor = config.highlightColor || '#10B981';
  return (
    <div
      style={{
        display: 'inline-flex',
        ...
        return (
          <div
            ...
            style={{
              ...
              padding: '6px 18px',
              borderRadius: 9999,
              background: isActive ? `linear-gradient(135deg, ${accentColor}, #059669)` : 'rgba(24, 24, 27, 0.72)',
              border: isActive ? '1px solid rgba(255, 255, 255, 0.3)' : '1px solid rgba(255, 255, 255, 0.14)',
              backdropFilter: 'blur(20px)',
              WebkitBackdropFilter: 'blur(20px)',
              boxShadow: isActive
                ? `0 8px 24px ${accentColor}55, inset 0 1px 0 rgba(255, 255, 255, 0.35)`
...
// Style: M3 ELEVATED CARD — Google Material 3 Level 2/3 Surface Container with active accent
const M3ElevatedCardStyle: React.FC<StyleProps> = ({caption, config, fontSize, activeWord}) => {
  const words = caption.words ?? [];
  const cardSize = Math.max(42, Math.min(fontSize * 0.88, 70));
  const accentColor = config.highlightColor || '#A855F7';
  return (
    <div
      style={{
        display: 'inline-flex',
          ...
          return (
            <span
              ...
              style={{
                ...
                fontWeight: isActive ? 900 : 700,
                fontFamily: config.fontFamily,
                color: isActive ? accentColor : '#F4EFF4',
                textShadow: isActive ? `0 0 16px ${accentColor}66` : '0 2px 8px rgba(0,0,0,0.5)',
