...
function ImageScene({ scene, progress }: { scene: Scene; progress: number }) {
  ...
  return (
    <AbsoluteFill style={{ opacity, backgroundColor: '#020617' }}>
      {scene.imageSrc ? (
        <div style={{ position: 'absolute', inset: -30, filter: 'blur(36px) brightness(0.6) saturate(1.25)', pointerEvents: 'none' }}>
          <Img src={resolveAsset(scene.imageSrc)} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        </div>
      ) : null}
      {scene.imageSrc ? (
