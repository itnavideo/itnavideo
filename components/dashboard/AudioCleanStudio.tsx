        ...
        <div className="rounded-[28px] border border-white/10 bg-[#1a1822] shadow-2xl overflow-hidden">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {audioCleanAnalysis.stats?.repeatedTakesCount > 0 && (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-500/20 bg-amber-500/10 px-3 py-1 text-xs font-bold text-amber-300">
                  <Scissors size={12} />
                  {audioCleanAnalysis.stats.repeatedTakesCount} Retakes
                </span>
              )}
              {audioCleanAnalysis.stats?.falseStartsCount && audioCleanAnalysis.stats.falseStartsCount > 0 ? (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-orange-500/20 bg-orange-500/10 px-3 py-1 text-xs font-bold text-orange-300">
                  <RotateCcw size={12} />
                  {audioCleanAnalysis.stats.falseStartsCount} False Starts
                </span>
              ) : null}
              {audioCleanAnalysis.stats?.mistakesCount && audioCleanAnalysis.stats.mistakesCount > 0 ? (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-rose-500/20 bg-rose-500/10 px-3 py-1 text-xs font-bold text-rose-300">
                  <AlertCircle size={12} />
                  {audioCleanAnalysis.stats.mistakesCount} Stumbles
                </span>
              ) : null}
              {audioCleanAnalysis.stats?.fillerCount > 0 && (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-purple-500/20 bg-purple-500/10 px-3 py-1 text-xs font-bold text-purple-300">
                  <Mic size={12} />
                  {audioCleanAnalysis.stats.fillerCount} Fillers
                </span>
              )}
            </div>
          ...
          <div className="bg-emerald-950/20 border-b border-emerald-500/20 px-6 py-2.5 flex items-center justify-between text-xs text-emerald-300">
            <div className="flex items-center gap-2">
              ...
              <span>
                <strong>Script Safeguard Active:</strong> Complete sentences and natural breathing are strictly preserved. Only speech stumbles, false starts, and repeated takes are flagged for removal.
              </span>
            </div>
            <span className="text-[11px] text-zinc-400 hidden sm:inline">
              Click any cut take to restore
            </span>
          </div>
                        ...
                        {sentences.map((sent, sIdx) => {
                          ...
                          return (
                            ...
                            >
                              {sent.text}
                              {isCut && (
                                <span className={`ml-1 inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider no-underline border ${
                                  sent.reason === 'false-start'
                                    ? 'bg-orange-500/20 text-orange-300 border-orange-500/40'
                                    : sent.reason === 'mistake' || sent.reason === 'stumble'
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <div className="space-y-1">
              ...
              <p className="text-xs text-[#cac4d0]">
                Preserves natural breathing & trims dead air &gt; 1.1s
              </p>
            </div>
            <div
              className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${
          ...
          <div
            onClick={() => handleToggleOption("removeFillers")}
            className="flex cursor-pointer items-start justify-between rounded-2xl border border-white/10 bg-[#22202c] p-4 transition hover:border-white/20"
          >
            <div className="space-y-1">
              <p className="text-sm font-semibold text-white">Remove Vocal Fillers</p>
              <p className="text-xs text-[#cac4d0]">
                Auto-cuts "um", "uh", "matlab", &amp; verbal fillers
              </p>
            </div>
            <div
              className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${
                audioCleanOptions.removeFillers ? "bg-[#381e72]" : "bg-zinc-700"
              }`}
            >
              <span
                className={`absolute top-1 h-4 w-4 rounded-full bg-[#d0bcff] transition-transform ${
                  audioCleanOptions.removeFillers ? "left-6" : "left-1"
                }`}
              />
            </div>
          </div>
