...
- ☁️ **Video Rendering**: Strictly AWS Remotion Lambda (`npm run reel:lambda:render` or production `/api/reels/jobs`). Compute runs in AWS Lambda only.
- ☁️ **Transcription**: Groq Whisper Cloud API (ultra-fast, cloud-based).
- ☁️ **AI Planning / Vision**: Google Gemini Cloud API / Groq Cloud API.
- ☁️ **Storage & Assets**: AWS S3 & Cloud CDN.
- ☁️ **Deployments**: Vercel Cloud (`npx vercel --prod`) & Remotion Lambda deploy (`npm run reel:lambda:deploy`).

### PERMITTED Local Operations (Ultra-Lightweight Only):
- ✅ Code editing, file inspection, and git commands.
- ✅ Static verification: `npx tsc --noEmit`
- ✅ Cloud deploy & trigger commands (`npx vercel --prod`, `npm run reel:lambda:deploy`, `npm run reel:lambda:render`).
