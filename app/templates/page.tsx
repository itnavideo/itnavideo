...
const templates = [
  {
    ...
    dashHref: "/dashboard?template=dynamic-creator-reel",
  ...
  },
  {
    ...
    dashHref: "/dashboard?template=auto-caption-reel",
  ...
  },
  {
    ...
    dashHref: "/dashboard?template=creator-background-replace",
  ...
  },
  {
    ...
    dashHref: "/dashboard?template=compare-explainer",
  ...
  },
  {
    ...
    dashHref: "/dashboard?template=auto-draw-explainer",
  ...
  },
  {
    ...
    dashHref: "/dashboard?template=long-video-promo",
...
export default function TemplatesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (category !== "all") result = result.filter((template) => template.category === category);
    if (search.trim()) {
      const q = search.toLowerCase();
      result = result.filter(
        (template) =>
          template.title.toLowerCase().includes(q) ||
          template.desc.toLowerCase().includes(q) ||
          ...
          <div className="grid gap-8 lg:grid-cols-[0.78fr_1fr] lg:items-end">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-base leading-7 text-slate-400 sm:text-lg">
                Itnavideo now focuses on six templates. No crowded library, no average formats - just the reel styles we can make feel premium and reliable.
              </p>
            </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-5 flex items-center justify-between gap-4">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
              {filtered.length} live templates
            </p>
              ...
              >
                <Link href={template.href} className="block">
                  <div className="relative aspect-[9/16] overflow-hidden bg-black">
                    <Image
                      src={template.image}
                      alt={`${template.title} output preview`}
                      fill
                    ...
                    <div className="absolute bottom-4 left-4 right-4">
                      ...
                      <p className="mt-1 text-[11px] font-bold uppercase tracking-wide text-slate-400">{template.input}</p>
                    </div>
                  </div>
                </Link>

                <div className="p-4">
                  <p className="min-h-[3rem] text-sm leading-6 text-slate-400">{template.desc}</p>
                  <div className="mt-4 flex gap-2">
                    <Link
                      href={template.dashHref}
          ...
          {filtered.length === 0 && (
            <div className="py-16 text-center">
              <p className="text-lg font-bold text-slate-500">No template matches your search.</p>
              <button
                onClick={() => {
                  setSearch("");
                  setCategory("all");
      ...
      <section className="border-t border-white/10 px-4 py-16 sm:px-6">
        <div className="mx-auto grid max-w-5xl gap-4 sm:grid-cols-3">
          {[
            ...
            ["Focused quality", "The team can improve six templates deeply instead of maintaining many weak formats."],
            ["Clear inputs", "Every template tells users exactly what to upload and what output to expect."],
          ].map(([title, body]) => (
