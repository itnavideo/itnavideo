...
function getPlannedRenderCreditUnits(mode: Mode, durationSeconds?: number, clipCount = 3) {
  try {
    ...
    const billableModes: BillableRenderMode[] = ["autoCaption", "compare", "longVideoPromo", "whiteboardVideo", "typographyVideo", "longVideoPro", "facelessVideo", "imageToVideoAi", "aiVideoGenerator"];
...
const modeConfig = {
  ...
  imageToVideoAi: {
    ...
    accept: "audio/*",
    supported: "MP3, WAV, M4A, AAC • Up to 30 minutes",
    bestResult: "Podcasts, storytelling, explainers & presentations in 16:9 format",
    uploadCta: "🎙️ Upload your voiceover / audio (Max 30 min)",
    icon: Film,
        ...
        <section id="studio-workflows" className="w-full scroll-mt-24 pt-2">
          ...
          {hasUserSelected && (
            <div id="video-creation-studio" className="mt-8 space-y-6 border-t border-border pt-6 scroll-mt-24">
              ...
              <div className={mode === "autoCaption" || mode === "creatorBackgroundReplace" || mode === "customAiReel" || mode === "longVideoPromo" || mode === "compare" || mode === "typographyVideo" || mode === "longVideoClips" || mode === "whiteboardVideo" || mode === "audioClean" || mode === "longVideoPro" || mode === "imageToVideoAi" ? "hidden" : "rounded-lg border border-border bg-card/[0.035] p-4"}>
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div>
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="inline-flex items-center rounded-full bg-amber-400/10 border border-amber-400/20 px-2.5 py-0.5 text-xs font-mono font-bold text-amber-400">
                    {recentRenders.length} Videos
                  </span>
                </div>
                <p className="mt-1 text-xs sm:text-sm text-zinc-400">
                  Your rendered videos are preserved in secure cloud storage for 48 hours for fast download.
                </p>
              </div>
            ...
            {recentRenders.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="mt-5 pt-3 border-t border-white/10 flex items-center justify-between gap-2">
                      <a
                        ...
                        download={`${render.title || "video"}.mp4`}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-amber-400 via-amber-500 to-orange-500 text-slate-950 px-4 py-1.5 text-xs font-black shadow-md hover:scale-[1.02] active:scale-95 transition-all cursor-pointer"
                      >
                        <Download size={13} />
              ...
              <div className="rounded-3xl border border-dashed border-white/15 bg-white/[0.02] p-10 sm:p-14 text-center">
                ...
                <h3 className="text-base font-bold text-white">No saved video renders yet</h3>
                <p className="mt-1.5 max-w-sm mx-auto text-xs text-zinc-400">
                  Pick a workflow from Video Types to create and render your first AI video with automatic subtitles.
                </p>
                <div className="mt-6">
                  <button
                    type="button"
